...
class ACDetect(nn.Module):
    ...
    def __init__(self, nc=2, ch=()):
        ...
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 3), ACmix(c2, c2, kernel_att=3, head=4, kernel_conv=3), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch
        )
        self.cv3 = (
            nn.ModuleList(nn.Sequential(Conv(x, c3, 3), ACmix(c3, c3, kernel_att=3, head=4, kernel_conv=3), nn.Conv2d(c3, self.nc, 1)) for x in ch)
            if self.legacy
            else nn.ModuleList(
                nn.Sequential(
